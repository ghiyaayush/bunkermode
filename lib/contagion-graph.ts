...
export type PoolArch = "shared" | "isolated" | "amm" | "staking" | "restaking" | "cdp" | "yield";
...
export const NODES: GraphNode[] = [
  ...
  { id:"curve",      label:"Curve Finance",  type:"protocol", chain:"ethereum", poolArch:"amm",       riskLevel:"medium", tvl:2100,  description:"Stablecoin/LST AMM. rsETH/ETH pool collapse causes price impact here first." },
  ...
  // Starknet
  { id:"ekubo",       label:"Ekubo",       type:"protocol", chain:"starknet", poolArch:"amm",   riskLevel:"medium", tvl:80, description:"Primary concentrated-liquidity AMM on Starknet. Singleton pool architecture — all pools share one contract. High composability but a singleton bug affects all pairs simultaneously." },
  { id:"troves",      label:"Troves",      type:"protocol", chain:"starknet", poolArch:"cdp",   riskLevel:"medium", tvl:55, description:"CDP (collateralised debt position) protocol on Starknet. Users lock ETH or STRK to mint a synthetic stablecoin. Sharp price drops cascade into forced liquidations, stressing Ekubo exit liquidity." },
  { id:"forgeyields", label:"ForgeYields", type:"protocol", chain:"starknet", poolArch:"yield", riskLevel:"medium", tvl:35, description:"Yield aggregator on Starknet. Routes deposited assets into Ekubo LP positions and Troves vaults to optimise returns. Composability dependency means a bug in either downstream protocol propagates here." },

  // ── Tokens ─────────────────────────────────────────────────────────────────

  // Ethereum
  { id:"eth",    label:"ETH",      type:"token", chain:"ethereum", tokenType:"native",     riskLevel:"low",      description:"Native Ethereum. No bridge or collateral dependency risk. Safe exit destination." },
  { id:"weth",   label:"WETH",     type:"token", chain:"ethereum", tokenType:"wrapped",    riskLevel:"low",      description:"Wrapped ETH. Locked in Aave when rsETH panic drove utilization to 100% — 7 hours before the official freeze." },
  { id:"usdc",   label:"USDC",     type:"token", chain:"ethereum", tokenType:"stablecoin", riskLevel:"low",      description:"Circle stablecoin. CCTP burn-and-mint bridge = no wrapped token risk. Priority-1 safe exit." },
  ...
  // Starknet
  { id:"strk",          label:"STRK",           type:"token", chain:"starknet", tokenType:"governance", riskLevel:"medium", description:"Starknet governance and fee token. Also used as CDP collateral in Troves." },
  { id:"eth-starknet",  label:"ETH (Starknet)", type:"token", chain:"starknet", tokenType:"wrapped",    riskLevel:"low",    description:"ETH bridged to Starknet via StarkGate ZK-proof. Low bridge risk due to validity proofs." },
  { id:"usdc-starknet", label:"USDC (Starknet)",type:"token", chain:"starknet", tokenType:"stablecoin", riskLevel:"low",    description:"USDC on Starknet via Circle + StarkGate. Primary stablecoin in Ekubo pools." },
...
export const EDGES: GraphEdge[] = [
  ...
  // Ekubo — singleton AMM
  { source:"eth-starknet",  target:"ekubo", type:"LIQUIDITY_POOL", weight:4, label:"ETH/USDC pool" },
  { source:"usdc-starknet", target:"ekubo", type:"LIQUIDITY_POOL", weight:4, label:"stablecoin pool" },
  { source:"strk",          target:"ekubo", type:"LIQUIDITY_POOL", weight:3, label:"STRK/ETH pool" },

  // Troves — CDP
  { source:"eth-starknet", target:"troves", type:"COLLATERAL_IN", weight:3, label:"CDP collateral" },
  { source:"strk",         target:"troves", type:"COLLATERAL_IN", weight:2, label:"CDP collateral" },
  { source:"troves",       target:"ekubo",  type:"LIQUIDITY_POOL",weight:2, label:"minted stablecoin LP" },

  // ForgeYields — aggregator
  { source:"eth-starknet",  target:"forgeyields", type:"YIELD_SOURCE", weight:3 },
  { source:"usdc-starknet", target:"forgeyields", type:"YIELD_SOURCE", weight:3 },
  { source:"forgeyields",   target:"ekubo",       type:"YIELD_SOURCE", weight:3, label:"LP position" },
  { source:"forgeyields",   target:"troves",      type:"YIELD_SOURCE", weight:2, label:"vault deposit" },
